...
class PageActions {
	...
	const result = await this.getProperty("//*[@id='resultDiv']",'innerText');
	expect(result).to.equal(msg);
    }
    
    async verifyTextonPage(text) {
	expect(await this.getProperty("//*[contains(text(),'"+text+"')]",'innerText')).to.not.be.null;
	...
	var fs = require('fs');
	var ff = '';
	if (global.webrowser === 'firefox') ff = 'ff_';
	if (global.headless){
